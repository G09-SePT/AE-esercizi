...
public class MainMemory implements Mic1Constants
{
	...
	public void poke()
	{
		if (write)
		{
			...
			{
				if (mic1sim.debug)
					DebugFrame.text.append("MEM: Write value 0x" + Integer.toHexString(data).toUpperCase()
				...
				{
					mic1sim.update = true;
					mic1sim.address = address;
				}
				if (mic1sim.stack)
				{
					mic1sim.update_Stack = true;
					mic1sim.address_Stack = address / 4;
				}
		...
		if ((main_memory_cl.getValue())[MIR.WRITE])
		{
			...
			if (mic1sim.debug)
				DebugFrame.text.append("MEM: Write value 0x" + Integer.toHexString(write_data).toUpperCase() +
		...
		if (read)
		{
			...
			} else
			{
				...
				mdr.store();
				if (mic1sim.debug)
					DebugFrame.text.append("MEM: Read value 0x" + Integer.toHexString(data).toUpperCase()
		...
		if ((main_memory_cl.getValue())[MIR.READ])
		{
			...
			if (mic1sim.debug)
				DebugFrame.text.append("MEM: Read from word# 0x" +
		...
		if (fetch)
		{
			...
			if (address < MEM_MAX && address >= 0)
			{
				...
				if (mic1sim.debug)
					DebugFrame.text.append("MEM: Fetch value 0x" + Integer.toHexString(data).toUpperCase()
							+ " from address 0x" + Integer.toHexString(address).toUpperCase() + "\n");
			} else
		...
		if ((main_memory_cl.getValue())[MIR.FETCH])
		{
			...
			if (mic1sim.debug)
				DebugFrame.text.append("MEM: Fetch from byte# 0x" +
