 ...
 * Name             		Date       		Comment
 * ---------------- 		---------- 	----------------------------------------
 * Claudio Bertoncello		1999.06.10 	added method getValue()
 * Simone Alciati			2000.04.01	added method setValue()
 */
...
public class PC extends TextField
{
	...
	public void store()
	{
		if (store_cl.getValue()[0])
		{
			value = in.getValue();
			if (mic1sim.debug)
				DebugFrame.text.append("PC: Store " + Integer.toHexString(value).toUpperCase() + "\n");
	...
	public void put()
	{
		if (put_cl.getValue()[0])
		{
			out.setValue(value);
			if (mic1sim.debug)
				DebugFrame.text.append("PC: Put " + Integer.toHexString(value).toUpperCase() + "\n");
			if (!mic1sim.run || cycle_count == COUNT_MAX)
			{
				setText("0x" + Integer.toHexString(value).toUpperCase());
				cycle_count = 0;
	...
	public void mem()
	{
		if (mem_cl.getValue()[MIR.FETCH])
		{
			...
			if (mic1sim.debug)
				DebugFrame.text.append("PC: Fetch byte " + Integer.toHexString(value).toUpperCase() + "\n");
			if (!mic1sim.run || cycle_count == COUNT_MAX)
			{
				setText("0x" + Integer.toHexString(value).toUpperCase());
				cycle_count = 0;
