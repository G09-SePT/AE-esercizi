...
public class mic1asm implements Mic1Constants
{
	...
	/**
	 * assembles a Mic1 Micro-Assembly Language program which it reads from in,
	 * and writes to out.
	 */
						 ...
						 PrintStream err)
	{
		...
		try
		{
			...
			//
			// we should verify that all labels which participate in if
			// statements
			// are always paired similarly (i.e., the "then" and "else" targets
			// must
			...
			for (int i = 0; i < mic1parse.statementCount; i++)
			{
				if (mic1parse.statements[i].isIf)
				{
					...
					if (pairOk)
					{
						...
						// if the elseLabel does not have a value (in the labels
						// hash table),
						// find the next available pair of addresses (by looking
						// in the array),
						...
						if (!errorFound)
							mic1parse.statements[i].i.NEXT_ADDRESS = address
									.intValue();
					}
				}
			}
			if (mic1parse.defaultStatement.isIf)
			{
				...
				// if the elseLabel does not have a value (in the labels hash
				// table),
				// find the next available pair of addresses (by looking in the
				// array),
				// and give both the gotoLabel and the elseLabel values
				...
				// update the NEXT_ADDRESS field for the statement.
				if (!errorFound)
					mic1parse.defaultStatement.i.NEXT_ADDRESS = address
							.intValue();
			}
			...
			// addresses. the labels are in the hash table with their assigned
			// addresses.
			...
			if (!errorFound)
			{
				for (int i = 0; i < mic1parse.statementCount; i++)
				{
					...
					else
					{
						...
						// different pass
						err
								.println("internal error: statement "
										+ i
